...
We have to performed taxonomic classification.
...
### II.F. Charge the R data image – ⚠️ Launch this step during the lessons to obtained the ASVs table previously produced (see the **II.E. Taxonomic classification** section)
...
Here, We choose to remove samples accounting less than 5,000 sequences.
```{r sample filtration, echo=TRUE}
ASV.table.raw.filt <- ASV.table.raw[,which(colSums(ASV.table.raw)>5000)]
```

Since some ASVs may correspond to sequencing errors (despite the previous precautions), we should choose to remove those characterized by a low amount of sequences. It exists different methods (not exhaustive):
...
To compare samples effectively, we should (though this is not mandatory) normalize them. It exists different methods (not exhaustive):
...
ASV.table.raw.filt.ss.t <- as.data.frame(t(ASV.table.raw.filt.ss)) # Create transposed OTU table
minsample <- min(rowSums(ASV.table.raw.filt.ss.t)) # save the abundance of the smallest sample in "minsample"
ASV.table.rare.filt.ss.t <- rrarefy(ASV.table.raw.filt.ss.t,sample=minsample) # Rarefy the transposed OTU table by the abundance of the smallest sample
ASV.table.rare.filt.ss <- t(ASV.table.rare.filt.ss.t) # transpose the transposed rarefied OTU table
colSums(ASV.table.rare.filt.ss) # Display the number of reads by sample
```
...
In many cases, these analyses use a distance matrix. Different types of distance may be used depending on the type and structure of the data. Here, for community table (ASVs table), the Bray-Curtis distance is the more adapted as it is less impacted by a large number of "0" values.

```{r Beta diversity, echo=TRUE}
# NMDS - Beta-diversity
dt <- as.data.frame(t(ASV.table.rare.filt.ss)) # We create a new data frame 'dt' that corresponded to the filtered, normalized and transposed OTU table.
...
text(x = par("usr")[2] - 0.02 * diff(par("usr")[1:2]), # Plot the stress value oin the plot
...
In some case, we want represent the taxonomic affiliation of the organisms present in each samples. A bar plot should help to visualize these data.

```{r Plot taxonomy, echo=TRUE, fig.height=12, fig.width=8}
ASV.table.rare.filt.ss.tax <- merge(ASV.table.rare.filt.ss, ASV.table %>% select(Taxonomy), by = "row.names") # We used the Taxonomy column of the "ASV.table" table to annotate the rarefied and filtered table
...
table_taxo <- ASV.table.rare.filt.ss.tax %>%
  select(all_of(starts_with("SAMEA")),Domain,Kingdom,Division) %>% # We select only several column : all sample labels and "Class" (3rd taxonomic rank)
  group_by(Domain,Kingdom,Division) %>% # We group all row by Class : rows with the same Class will be combine in one row
  summarise_all(sum) %>% # Sequences account of rows of the same Class will be sums up
  gather(SAMEA_ID,Abundance,-Domain,-Kingdom,-Division) # We transpose the contingency table to edit a new table with three columns : Class | Amplicon | Abundance of reads
